...
#define QUINTIC_NO_INTERSECTION 3.4e38
...
// than that the array is sorted. The last entry is always QUINTIC_NO_INTERSECTION.
void quintic_roots_cubic_deflate
(
    out float out_roots[6], 
    float poly[6], 
    float begin, 
...
void quintic_roots_deflate(
    ...
    for (int degree = 3; degree <= 5; ++degree) 
    {
        ...
        float defl_poly[6] = deriv_poly; int num_roots = 0; 
        ...
        for (int i = 0; i <= 4; ++i) 
        {
            ...
            if (quintic_root_newton_bisection(current_root, begin_value, deriv_poly, current_begin, current_end, begin_value, tolerance))
            {
                ...
                // Increment root count until there are two brackets left
                if (i < 4) num_roots++;
...
bool quintic_has_root(
    ...
    for (int degree = 3; degree <= 4; ++degree) 
    {
        ...
        for (int i = 0; i <= 4; ++i) 
        {
            ...
            // Try to find a root
            if (quartic_root_newton_bisection(current_root, begin_value, deriv_poly, current_begin, current_end, begin_value, tolerance))
            {
