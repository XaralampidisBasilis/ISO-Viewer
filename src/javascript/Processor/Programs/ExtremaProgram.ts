...
class ExtremaMapProgram implements GPGPUProgram 
{
    variableNames = ['A']
    outputShape: number[]
    userCode: string

    constructor(inputShape: number[], inputStride: number, inputMethod: string) 
    {
        this.outputShape = inputShape.map((inputDim) => Math.ceil((inputDim + 1) / inputStride))
        this.outputShape[3] = 2        
        this.userCode = (inputMethod == 'trilinear') 
        ? ExtremaMapProgram.trilinearCode(inputShape, inputStride) 
        : ExtremaMapProgram.tricubicCode(inputShape, inputStride) 
    }

    static trilinearCode(inputShape: number[], inputStride: number)  
    {   
        return `
    ...
    static tricubicCode(inputShape: number[], inputStride: number)
    {
        return`
...
export function computeExtremaMap(inputTensor: tf.Tensor, inputStride: number, inputMethod = 'tricubic') : tf.Tensor4D
{
    const program = new ExtremaMapProgram(inputTensor.shape, inputStride, inputMethod)
