...
const userCode = (inputShape: number[], inputStride: number) => `
    ...
    vec2 blockExtrema(int X, int Y, int Z)
    {
        ...
        int xm = X * ${inputStride} - 1;
        int ym = Y * ${inputStride} - 1;
        int zm = Z * ${inputStride} - 1;

        xm = clamp(xm, 0, ${inputShape[0] - 1});
        ym = clamp(ym, 0, ${inputShape[1] - 1});
        zm = clamp(zm, 0, ${inputShape[2] - 1});
        
        // compute exclusive max voxel indices of block
        int xM = xm + ${inputStride};
        int yM = ym + ${inputStride};
        int zM = zm + ${inputStride};
...
const userCode2 = (inputShape: number[], inputStride: number) => `
...
class BlockBersteinExtremaProgram implements GPGPUProgram 
{
    ...
    constructor(inputShape: number[], inputStride: number) 
    {
        ...
        this.userCode = userCode(inputShape, inputStride)
    }
}
