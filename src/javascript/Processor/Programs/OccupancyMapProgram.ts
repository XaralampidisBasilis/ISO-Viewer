...
export class OccupancyMapProgram implements GPGPUProgram 
{
    variableNames = ['A'];
    outputShape: number[];
    userCode: string;
...
export function computeOccupancyMap(input: tf.Tensor4D, inputThreshold: number): tf.Tensor4D 
{
  const backend = tf.backend() as MathBackendWebGL;
  const program = new OccupancyMapProgram(input.shape as [number, number, number, number], inputThreshold);
