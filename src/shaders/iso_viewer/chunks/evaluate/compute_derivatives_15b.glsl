...
vec3 x_samples = vec3(
    samples[0] + samples[2] + samples[3] + samples[4] + samples[8],
    samples[10] + samples[11] + samples[12] + samples[13] + samples[14],
    samples[1] + samples[5] + samples[6] + samples[7] + samples[9]
);

vec3 y_samples = vec3(
    samples[0] + samples[1] + samples[3] + samples[5] + samples[10],
    samples[8] + samples[9] + samples[12] + samples[13] + samples[14],
    samples[2] + samples[4] + samples[6] + samples[7] + samples[11]
);

vec3 z_samples = vec3(
    samples[0] + samples[1] + samples[2] + samples[6] + samples[12],
    samples[8] + samples[9] + samples[10] + samples[11] + samples[14],
    samples[3] + samples[4] + samples[5] + samples[7] + samples[13]
);
...
vec3 gradient;
gradient[0] = (x_samples.z - x_samples.x) * 0.5; 
gradient[1] = (y_samples.z - y_samples.x) * 0.5; 
gradient[2] = (z_samples.z - z_samples.x) * 0.5; 

mat3 hessian;
// pure second order partial derivatives with central differences
hessian[0][0] = dot(x_samples, vec3(1.0, -2.0, 1.0));
hessian[1][1] = dot(y_samples, vec3(1.0, -2.0, 1.0));
hessian[2][2] = dot(z_samples, vec3(1.0, -2.0, 1.0));
